...
class LeetCodeExecutor implements Disposable {
    ...
    public async meetRequirements(context: ExtensionContext): Promise<boolean> {
        ...
        try {
            ...
            const choice: MessageItem | undefined = await window.showErrorMessage(
                "This extension needs Node.js installed in environment path",
                DialogOptions.open,
            );
    ...
    public async submitSolution(filePath: string): Promise<string> {
        try {
            return await this.executeCommandWithProgressEx("Submitting...", this.nodeExecutable, [await this.getLeetCodeBinaryPath(), "submit", `"${filePath}"`]);
        } catch (error) {
            if (error.result) {
                return error.result;
            }
    ...
    public async testSolution(filePath: string, testString?: string): Promise<string> {
        if (testString) {
            return await this.executeCommandWithProgressEx("Submitting...", this.nodeExecutable, [await this.getLeetCodeBinaryPath(), "test", `"${filePath}"`, "-t", `${testString}`]);
        }
        return await this.executeCommandWithProgressEx("Submitting...", this.nodeExecutable, [await this.getLeetCodeBinaryPath(), "test", `"${filePath}"`]);
    }
